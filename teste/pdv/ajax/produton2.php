...
function gerarIdUnico($prefixo = 'ref_')
{
	// Gerar uma string aleatória com 20 caracteres
	$randomString = bin2hex(random_bytes(10)); // 10 bytes = 20 caracteres hexadecimais

	// Retornar o ID único com o prefixo
	return $prefixo . $randomString;
}

// Exemplo de uso
$referencia = gerarIdUnico();
...
<script type="text/javascript">
	arrayOpcionais = []; // Inicia o Array para fazer as verifica��es
...
<center><img src="../img/fotos_produtos/<?php if (!$cpro->foto) {
											echo "off.jpg";
										} else {
											print $cpro->foto;
										} ?>" style="width:100%;" class="img-thumbnail"></center>
...
<form action="ajax/produtoi.php" method="post">
	...
	<input type="hidden" name="idpedido" value="<?php print $idPedido; ?>">
	<input type="hidden" name="iduser" value="<?php print $cpro->idu; ?>">
	<input type="hidden" name="referencia" value="<?php echo $referencia; ?>">


	<!-- Criar um Verificação de integridade na Aplicação para o nome do produto -->
	<input type="hidden" name="nome_produto" value="<?php print $cpro->nome; ?>">


	<?php if ($opcionaisxk >= 1) {
		include_once('meioameio.php');
	} ?>
	<?php if ($qntta >= '1') {
		include_once('tamanhos.php');
	} ?>
	<?php if ($qntgrupos >= '1') {
		include_once('opcionais.php');
	} ?>
