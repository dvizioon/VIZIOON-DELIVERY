...
?>
		...
		<div class="slim-card-title"
			...
			style="color:#CC3300"
			<?php } elseif ($grupo_opx->obrigatorio == 2) { ?>
			style="color:#0066FF"
			<?php } ?>>
			<?php print $grupo_opx->nomegrupo; ?> -
			Até <?php print $grupo_opx->quantidade; ?>
			<?php echo ($grupo_opx->quantidade > 1) ? "opções" : "opção"; ?>
			<?php if ($grupo_opx->obrigatorio == 1) { ?>
		...
		<div class="row">
			...
			?>
				...
				<div class="col-8 mg-t-10">
					<?php if ($grupo_opx->obrigatorio == 1) { ?>
						<input type='radio' name="opcionais[<?php print $grupo_opx->Id; ?>]"
							value="<?php print $opc_dadosx->opnome; ?>,<?php print $opc_dadosx->valor; ?>," required>
						- <span><?php print $opc_dadosx->opnome; ?></span>
					<?php } ?>

					<?php if ($grupo_opx->obrigatorio == 2) { ?>
						<input type='checkbox' name="opcionais[<?php print $opc_dadosx->id; ?>]"
							value="<?php print $opc_dadosx->opnome; ?>,<?php print $opc_dadosx->valor; ?>,"
							id="<?php print $grupo_opx->Id; ?>">
						- <span><?php print $opc_dadosx->opnome; ?></span>
