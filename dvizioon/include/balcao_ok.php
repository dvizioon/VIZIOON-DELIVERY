...
if (isset($_POST["totalg"])) {
	...
	$obs = isset($obs) ? $obs : '';
	$data_nascimento = $_POST['data_nascimento'] ?? "0000-00-00";
...
");
	...
	// 	if ($registro) {
	// 		// Verificamos se os dados são diferentes dos existentes no banco (exceto data_nascimento)
	// 		$dadosAtualizados = [];

	// 		// Verificação de cada campo para atualização (exceto data_nascimento)
	...
	// 		}
	// 		if ($registro['primeiro_nome'] !== $nome) {
	// 			$dadosAtualizados['primeiro_nome'] = $nome;
	// 		}
	...
	// 		}
	// 	} else {
	// 		// Se o telefone não existir, inserimos os dados, incluindo data_nascimento
	// 		$insertDadosRegistro = $connect->prepare("INSERT INTO registroDados (telefone, idu, nome, bairro, endereco, complemento, cep, casa, primeiro_nome, data_nascimento) 
	//                                  VALUES (:telefone, :idu, :nome, :bairro, :endereco, :complemento, :cep, :casa, :primeiro_nome, :data_nascimento)");
	...
	// 			'casa' => $numero,
	// 			'primeiro_nome' => $nome,
	// 			'data_nascimento' => $data_nascimento  // Inserindo data de nascimento
	// 		]);
	// 	}
	// }
