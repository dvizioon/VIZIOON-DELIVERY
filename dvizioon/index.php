		...
		$(document).ready(function() {
			$(document).on('click', '.view_data', function() {
				...
				if (user_id !== '') {
					var dados = {
						user_id: user_id,
						id_pedido: <?php echo json_encode($_SESSION['id_cliente']); ?>
					};
	...
	<script>
		$(document).ready(function() {
			...
			function limpa_formulário_cep() {
				...
				$("#rua").val("").prop('disabled', true); // Desabilita o campo rua
				$("#bairro").val("");
				$("#cidade").val("");
				$("#uf").val("");
				$("#ibge").val("");
			...
			// Desativa o campo rua no início
			...
			$("#cep").blur(function() {
				...
				if (cep != "") {
					...
					if (validacep.test(cep)) {
						...
						$.getJSON("https://viacep.com.br/ws/" + cep + "/json/?callback=?", function(dados) {
							...
							if (!("erro" in dados)) {
								...
								$("#rua").val(dados.logradouro).prop('disabled', false); // Ativa o campo rua
								...
								$("#rua").prop('disabled', false).attr('placeholder', 'Preencha seu Endereço...');
							}
						});
					} else {
					...
					// CEP sem valor, limpa formulário e libera o campo rua para preenchimento manual
					limpa_formulário_cep();
					$("#rua").prop('disabled', false).attr('placeholder', 'Preencha seu Endereço...');
				}
			});
