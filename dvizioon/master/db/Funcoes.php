...
if (isset($_POST["cadmesa"])) {
	...
	if ($cadfunc) {
		header("location: mesas.php?ok=ok");
		exit;
	} else {
		header("location: mesas.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["cadfuncionarios"])) {
	...
	$perm_pdv = isset($_POST['perm_pdv']) ? 'Sim' : 'Não';
	$perm_desborad = isset($_POST['perm_desborad']) ? 'Sim' : 'Não';
	$perm_balcao = isset($_POST['perm_balcao']) ? 'Sim' : 'Não';
	$perm_mesa = isset($_POST['perm_mesa']) ? 'Sim' : 'Não';

	// Inserir no banco de dados
	$cadfunc = $connect->query("
        INSERT INTO funcionarios (idu, nome, login, senha, acesso, perm_pdv, perm_desborad, perm_balcao, perm_mesa) 
        VALUES ('$cod_id', '$cad_nome', '$cad_login', '$cad_senha', '$cad_acesso', '$perm_pdv', '$perm_desborad', '$perm_balcao', '$perm_mesa')
    ");

	if ($cadfunc) {
		header("location: funcionarios.php?ok=ok");
		exit;
	} else {
		header("location: funcionarios.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["editarempresa"])) {
	...
	if ($editarcad) {
		header("location: dadosempresa.php?emp=&ok=");
		exit;
	} else {
		header("location: dadosempresa.php?emp=&erro=erro");
		exit;
	}
}
...
if (isset($_POST["atendimento"])) {
	...
	if ($editarcad) {
		header("location: bannerelogo.php?ok=ok");
		exit;
	} else {
		header("location: bannerelogo.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["cadbairro"])) {
	...
	if ($cadbairro) {
		header("location: bairros.php?ok=ok");
		exit;
	} else {
		header("location: bairros.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["cadcate"])) {
	...
	if ($cadcat) {
		header("location: categorias.php?ok=ok");
		exit;
	} else {
		header("location: categorias.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["editcate"])) {
	...
	if ($editarcad) {
		header("location: categorias.php?ok=123");
		exit;
	} else {
		header("location: categorias.php?erro=erro");
		exit;
...
if (isset($_POST["cadpro"])) {
	...
	if ($novocad) {
		header("location: produtos.php?ok=ok");
		exit;
	} else {
		header("location: produtos.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["variacoes"])) {
	...
	if ($novocadv) {
		header("location: variacoes.php?idpp=" . $codvar . "&ok=ok");
		exit;
	} else {
		header("location: variacoes.php?idpp=" . $codvar . "&erro=ok");
		exit;
	}
}
...
if (isset($_POST["opcionaisv"])) {
	...
	if ($novocado) {
		header("location: opcionais.php?idpp=" . $codvar . "&ok=ok");
		exit;
	} else {
		header("location: opcionais.php?idpp=" . $codvar . "&erro=ok");
		exit;
	}
}
...
if (isset($_POST["limitopc"])) {
	...
	if ($limitopc) {
		header("location: opcionais.php?idpp=$codpr");
		exit;
	} else {
		header("location: opcionais.php?idpp=$codpr&erro=erro");
		exit;
	}
}
...
if (isset($_POST["edlimitopc"])) {
	...
	if ($editlimi) {
		header("location: opcionais.php?idpp=$codprx&ok=ok");
		exit;
	} else {
		header("location: opcionais.php?idpp=$codprx&erro=erro");
		exit;
	}
}
...
if (isset($_POST["cadgrupo"])) {
	...
	if ($cadgrupo) {
		header("location: grupos.php?ok=ok");
		exit;
	} else {
		header("location: grupos.php?erro=erro");
		exit;
	}
}
...
if (isset($_POST["deletarcatx"])) {
	...
	if ($delb) {
		header("location: opgrupo.php?idpp=$idpp&ok=ok");
		exit;
	} else {
		header("location: opgrupo.php?idpp=$idpp&erro=erro");
		exit;
	}
}
...
if (isset($_POST["cadgruposx"])) {
	...
	if ($cadgrupof) {
		header("location: opgrupo.php?idpp=" . $cadgruposxy . "&ok=ok");
		exit;
	} else {
		header("location: opgrupo.php?idpp=" . $cadgruposxy . "&erro=erro");
		exit;
	}
}
...
if (isset($_POST['atualizarFuncionario'])) {
	...
	$acesso = $_POST['cad_acesso'];

	// Verificando as permissões
	$perm_pdv = isset($_POST['perm_pdv']) ? 1 : 0;
	$perm_desborad = isset($_POST['perm_desborad']) ? 1 : 0;
	$perm_balcao = isset($_POST['perm_balcao']) ? 1 : 0;
	$perm_mesa = isset($_POST['perm_mesa']) ? 1 : 0;

	// Preparar a consulta de atualização
	$stmt = $connect->prepare("UPDATE funcionarios SET nome = :nome, login = :login, acesso = :acesso, perm_pdv = :perm_pdv, perm_desborad = :perm_desborad, perm_balcao = :perm_balcao, perm_mesa = :perm_mesa WHERE id = :id");
	...
	if ($stmt->execute([
		...
		':acesso' => $acesso,
		':perm_pdv' => $perm_pdv,
		':perm_desborad' => $perm_desborad,
		':perm_balcao' => $perm_balcao,
		':perm_mesa' => $perm_mesa,
		':id' => $id_funcionario
	])) {
		// Redireciona com mensagem de sucesso
		header("Location: ./funcionarios.php?ok=success");
		exit();
	} else {
		// Redireciona com mensagem de erro
		header("Location: ./funcionarios.php?error=error");
		exit();
	}
}
